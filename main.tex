...
heuristic search algorithm for solving SPP. For completeness, we provide below a short description of the \astar{} algorithm, and Algorithm~\ref{alg:astar} shows its pseudo code. 
\astar{} is maintains two lists of states: \open{} and \closed{}. 
Initially, \closed{} is empty and \open{} contains only the start state. 
Every node that is added to \open{} is associated with a $g$ value and an $f$ value. 
...
This heuristic estimate is denoted by $h(n)$, and there is an abundance of prior work in the literature on how to develop efficient heuristics. 
