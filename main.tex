...
In \kgs{}, however, there are $k$ heuristic values for each node $n$, $h_1(n),\ldots,h_k(n)$ and 
thus each node has multiple  $f$ values, $f_1(n),\ldots,f_k(n)$, one per goal. We consider in this section the following two options for aggregating these $f$ values to a single $F_n$ value that will be used by \kastar{} to prioritize \open{}. 

\begin{align}
F_{min}(n)=&\min_{i\in [1,k]}f_i(n) & (\textbf{Min-f})\\ 
F_{max}(n)=&\max_{i\in [1,k]}f_i(n) & (\textbf{Max-f})\\ 
\end{align}
