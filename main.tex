...
\begin{tabular}{|m{2cm}|m{5.5cm}|}
        ...
        $Gen_i(\text{\kastar{}})$ & The set of  states generated by \kastar{} until $g_i$ is expanded \\
        Exp($X$) 	& The set fo states expanded by algorithm $X$ \\
...
summarizes some of the notations used in this paper.
...
cost 5 and has $h_2(g_1)=0$. If $h$ was consistent then $h_2(A)-w((A,g_1))\leq
h_2(g_1)$ (recall that $w((A,g_1))$ is the cost of the edge between $A$ and $g_1$).
...
Running \kastarmax{} will return $k$ paths $\pi_1,\ldots, \pi_k$ such that $\forall i\in[1,k]$ $\pi_i$ is a lowest cost path from $s$ to $g_i$. \label{cor:max-f}
\end{corollary}
...
Algorithm~\ref{alg:kastar}). This means states generated from hereinafter will not compute the heuristic for that goal. However, the states in \open{} 
...
Let $C_r$ be the cost of re-computing the $F$ value (using either \minf{} or \maxf{}) for a state and updating its position in \open{} accordingly. 
...
Without loss of generality, assume that $g_i$ is the  $i^{th}$ goal that has been found, and denote by $Gen_i(\text{\kastar{}})$ the set of  states generated by \kastar{} until $g_i$ is expanded. After the first goal is found, there are at most $|Gen_1(\text{\kastar{}})|$ states in \open{}. Thus, reordering them will require a computational cost of at most $|Gen_1(\text{\kastar{}})|\cdot C_r$. After the second goal is found, there
are at most $|Gen_2(\text{\kastar{}})|$ states in \open{} so reordering \open{} requires at most
$|Gen_2(\text{\kastar{}})|\cdot C_r$. Following this reasoning, the total computational
cost incurred by Eager \kastar{} for reordering \open{} after every goal is found is at most
\begin{equation}
O_{eager}=\sum_{i=1}^{k-1} |Gen_i(\text{\kastar{}})|\cdot C_r
\label{eq:re-sort-cost}
\end{equation}
We denote this extra cost by $O_{eager}$.
An important question is whether $O_{eager}$ is significant with respect to the overall runtime of \kastar{}. Observe that $Gen_k(\text{\kastar{}})$ is the set of all states generated by \kastar{}, and thus the runtime of \kastar{} is at least
...
For example, consider a state $n$ with $F$ value that is computed from the vector of $f$ values $\textbf{f}(n)=(10,5,3)$. When using \kastarmin{}, $F(n)=f_3(n)=3$. Therefore, the $F$ value of $n$ will only change if the goal $g_3$ is expanded and consequently removed from the set of active goals. Importantly, there is no need to re-compute it after goals $g_1$ or $g_2$ are expanded. More generally, after a goal $g_i$ is expanded there is no need to re-compute the $F$ value
...
Lazy \kastar{} poses different properties when using \maxf{} 
and when using \minf{}. In particular, Lazy \kastar{} brings no benefit to \kastarmax{} while it is very helpful for \kastarmin{}. Thus, we analyze them separately.  %n particular, the lazy approach only brings benefit to \kastarmin{} 
...
Consider running Lazy \kastarmin{} on the graph depicted in Figure~\ref{fig:lazy}(b). Assume we have a perfect heuristic for 
...
The relationship between Exp(\kxastar{}), Exp(\kastarmin), and Exp(\kastarmax) is given in the following observation.
\begin{observation}
    Up to tie-breaking, \kastarmin{} expands exactly the same set of states as \kxastar{},
...
\noindent {\bf Reordering in \open{}} ($C_r$).
\open{} gets reordered after every goal is found (except for the last goal). 
It is difficult to analyze exactly the runtime incurred due to these reordering operations. 
Thus, we focus this part of our analysis on the simple Eager version of \kastarmin{} (Section~\ref{sec:eager}), which will serve as a rough, upper bounding, approximation of the associated runtimes in the smart Eager and Lazy versions of \kastarmin{}.  
...
entire state space graph is given as input, and so the runtime of a single
...
As expected, the runtime of UCS is unaffected by the number of pivots, since it does not use a heuristic. Also, we observe that as the number of goals increase, the benefit of using heuristic search over plain UCS diminishes. Eventually, when solving for 128 goals, UCS dominates both \kastarmin{} and \kxastar{}. %solving for large number of goals eventually 

Next, consider the impact of adding pivots -- i.e., using a stronger but more costly heuristic. 
For the \kxastar{}, we observe that adding pivots always helps reducing runtime. For \kastarmin{}, however, adding pivots improves runtime only for cases when the number of goals was not very large. For example, when $k=32$, using more than 4 pivots actually degrades the performance of \kastarmin{}. These results conform with out theoretical analysis: adding pivots increase $C_h$, which is incurred more times in \kastarmin{} than in \kxastar{}. 
